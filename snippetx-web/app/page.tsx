...
export default function SnippetX() {
  ...
  useEffect(() => { fetchSnippets(selectedCategory, debouncedSearch); }, [selectedCategory, debouncedSearch]);
  ...
  const onAuthSuccess = () => { setIsLoggedIn(true); fetchUser(); fetchSnippets(); };
  ...
  const handleLogout = () => {
    ...
    setIsLoggedIn(false); setUser(null); setSelectedSnippet(null); setIsMobileDetailView(false); fetchSnippets();
  };
      ...
      >
        ...
        <div className="p-4 space-y-4 shrink-0">
          <div className="relative group">
            ...
            <input type="text" placeholder="搜索标题、描述或代码内容..." className={`w-full py-2 pl-9 pr-4 text-sm focus:outline-none transition-all ${theme === 'modern' ? 'bg-[#0d1117] border border-gray-800 rounded-xl focus:border-blue-500' : 'bg-white/10 border-none rounded-md'}`} value={searchText} onChange={(e) => setSearchText(e.target.value)} />
              ...
              <div className={`flex flex-col min-w-0 overflow-hidden ${theme === 'smartisan' ? 'smartisan-card rounded-xl' : theme === 'classic-ios' ? 'border border-gray-300 rounded-lg shadow-inner' : 'rounded-2xl border border-gray-800 shadow-2xl'}`} style={{ backgroundColor: 'var(--bg-card)' }}>
                ...
                <div className="flex-1 overflow-auto p-4 custom-scrollbar">
                  <div className="min-w-fit">
                    <SyntaxHighlighter language={selectedSnippet.language.toLowerCase()} style={getHighlighterStyle() as any} customStyle={{ fontSize: "14px", margin: 0, padding: 0 }} showLineNumbers={true} wrapLines={false}>
